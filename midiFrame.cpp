...
MidiFrame::MidiFrame(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, MidiFile *file) : wxPanel(parent, id, pos, size)
{
	midi = file;
	int index = midi->getTrackIndex();

	midi->getTrackFrameList(index)->SetBackgroundStyle(wxBG_STYLE_PAINT);

	midi->getTrackFrameList(index)->Bind(wxEVT_PAINT, &MidiFrame::OnPaint, midi->getTrackFrameList(index));
	midi->getTrackFrameList(index)->Bind(wxEVT_LEFT_DOWN, &MidiFrame::OnMouseDown, midi->getTrackFrameList(index));
	midi->getTrackFrameList(index)->Bind(wxEVT_MOTION, &MidiFrame::OnMouseMove, midi->getTrackFrameList(index));
	midi->getTrackFrameList(index)->Bind(wxEVT_LEFT_UP, &MidiFrame::OnMouseUp, midi->getTrackFrameList(index));
	midi->getTrackFrameList(index)->Bind(wxEVT_LEAVE_WINDOW, &MidiFrame::OnMouseLeave, midi->getTrackFrameList(index));
	midi->getTrackFrameList(index)->Bind(wxEVT_RIGHT_DOWN, &MidiFrame::OnMouseEvent, midi->getTrackFrameList(index));


	midi->getTrackFrameList(index)->draggedObj = nullptr;
	midi->getTrackFrameList(index)->shouldExtend = false;

	midi->getTrackFrameList(index)->selected = false;
}
...
void MidiFrame::addNote(int width, int height, int centerX, int centerY, wxColor color)
{
	...
	int index = midi->getTrackIndex();
	midi->getTrackFrameList(index)->noteList.push_back(obj);
...
{
	int index = midi->getTrackIndex();
	if (!midi->getTrackFrameList(index)->noteList.empty() && draggedObj == nullptr)
	{
		midi->getTrackFrameList(index)->noteList.pop_back();
...
{
	int index = midi->getTrackIndex();
	wxAutoBufferedPaintDC dc(midi->getTrackFrameList(index));
	dc.Clear();
...
void MidiFrame::sendNoteAddedEvent()
{
	...
	event.SetEventObject(midi->getTrackFrameList(index));
